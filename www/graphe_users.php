...
while($dateTmpGraphe <= $dateFinGraphe) {
	if($_SESSION['stats_users']['abscisse_echelle'] == 'jour') {
		...
		$interval = 'week';
		$donnees[$smarty->get_config_vars('planning_semaine') .  $dateTmpGraphe->format('W Y')] = array();
	...
	if(!is_null($periode->date_fin)) {
		while($tmpDateTache <= $finTache) {
			...
			if($_SESSION['stats_users']['abscisse_echelle'] == 'jour') {
				...
				$cleTableau = $smarty->get_config_vars('planning_semaine') .  $tmpDateTache->format('W Y');
			} elseif ($_SESSION['stats_users']['abscisse_echelle'] == 'mois') {
...
foreach ($donneesOrdonnee as $user_id => $valeurs) {
	...
	if(is_null($couleurUser) || $couleurUser == 'FFFFFF') {
		$couleurUser = '000000';
	}
